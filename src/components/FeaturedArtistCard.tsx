...
const FeaturedArtistCard = ({
  ...
  return (
    <Link to={`/artist/${id}`} className="block h-full">
      <Card className="glass-card overflow-hidden hover-lift h-full group transition-all duration-300 hover:shadow-xl">
        <div className="relative aspect-square overflow-hidden">
          ...
          <div className="absolute top-2 sm:top-3 right-2 sm:right-3 flex gap-1 sm:gap-2 flex-col sm:flex-row">
            {verified && (
              <span className="badge badge-verified text-xs flex items-center justify-center whitespace-nowrap animate-pulse-glow">
            ...
            {premium && (
              <span className="badge badge-premium text-xs flex items-center justify-center whitespace-nowrap animate-pulse-glow">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-2.5 h-2.5 mr-0.5 sm:w-3 sm:h-3 sm:mr-1">
          ...
          {/* Hover stats overlay */}
          <div className="absolute bottom-2 left-2 right-2 bg-black/60 backdrop-blur-sm rounded-lg p-2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
        ...
        <CardContent className="p-2 sm:p-4">
          <h3 className="font-heading font-semibold text-sm sm:text-lg line-clamp-1 group-hover:text-artswarit-purple transition-colors duration-300">{name}</h3>
