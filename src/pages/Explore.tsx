...
const Explore = () => {
  ...
  const filteredArtwork = allArtwork.filter(artwork => {
    const matchesSearch = artwork.title.toLowerCase().includes(searchTerm.toLowerCase()) || artwork.artist.toLowerCase().includes(searchTerm.toLowerCase()) || artwork.category.toLowerCase().includes(searchTerm.toLowerCase());
          ...
          <div className="mb-8 mx-0 my-0 rounded-none">
            ...
            <div className="bg-white/60 backdrop-blur-md p-6 rounded-lg shadow-sm border border-blue-100 mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {categories.map(category => <Button key={category} variant={selectedCategory === category ? "default" : "outline"} size="sm" onClick={() => setSelectedCategory(category)} className={selectedCategory === category ? "bg-gradient-to-r from-artswarit-purple to-blue-500 border-none" : "border-blue-200"}>
                        {category}
                      </Button>)}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {mediums.map(medium => <Button key={medium} variant={selectedMedium === medium ? "default" : "outline"} size="sm" onClick={() => setSelectedMedium(medium)} className={selectedMedium === medium ? "bg-gradient-to-r from-artswarit-purple to-blue-500 border-none" : "border-blue-200"}>
                        {medium}
                      </Button>)}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <Slider id="price-range" defaultValue={[0, 1000]} max={1000} step={50} value={priceRange} onValueChange={setPriceRange} className="my-4" />
                </div>
              </div>
              ...
              <div>
                ...
                <RadioGroup defaultValue="latest" value={sortBy} onValueChange={setSortBy} className="flex flex-wrap gap-4">
            ...
            {filteredArtwork.length > 0 ? <div className="mt-8">
                <Carousel opts={{
              align: "start",
              loop: true
            }} className="w-full">
                  <CarouselContent>
                    {sortedArtwork.map(artwork => <CarouselItem key={artwork.id} className="md:basis-2/3 lg:basis-1/2 pl-4">
                        <div className="artwork-card group h-[500px] md:h-[600px] rounded-2xl overflow-hidden shadow-lg transition-all duration-500 neo-blur-sm relative">
                          <img src={artwork.imageUrl} alt={artwork.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
              ...
              </div> : <div className="text-center py-12 bg-white/50 backdrop-blur-sm rounded-lg">
